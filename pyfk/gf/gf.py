...
def calculate_gf(config: Optional[Config] = None) -> None:
    ...
    t0_vp, td_vp, p0_vp, pd_vp = taup(config.src_layer, config.rcv_layer,
                                      config.model.th.astype(np.float64), config.model.vp.astype(np.float64), config.receiver_distance.astype(np.float64))
    t0_vs, td_vs, p0_vs, pd_vs = taup(config.src_layer, config.rcv_layer,
                                      config.model.th.astype(np.float64), config.model.vs.astype(np.float64), config.receiver_distance.astype(np.float64))
    ...
    # ! note, we directly use model, src_layer, rcv_layer, as they might be fliped and we don't want to change the config
...
def calculate_gf_source(srcType: str, model: SeisModel, flip: bool) -> np.ndarray:
...
class GF(object):
    def __init__(self, gf_data: Tuple[np.ndarray, ...], gf_head: dict) -> None:
        # * the gf will be a simple list, and its number should be determined by srcType
        self._gf = []
