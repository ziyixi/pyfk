...
cdef double complex[:,:,:] waveform_integration(int nfft2, double dw, double pmin, double dk, double kc, double pmax,
                           ...
                           int src_layer, int rcv_layer, str updn, double epsilon):
    # * get jv
    ...
    for ik in range(wc1, nfft2):
        ...
        if flip:
            ...
            flip_val = 1
        for i in range(n):
            u = kernel(k, u, kp, ks, aaa, bbb, ccc, eee, ggg, zzz, sss, temppp, mu, si,
                       thickness, src_type, src_layer, rcv_layer, updn, epsilon)
...
cdef double complex[:,:] kernel(double k, double complex[:,:] u, double complex[:] kp, double complex[:] ks,
    ...
    for ilayer in range(len(thickness), -1, -1):
        ...
        if ilayer == len(thickness)-1 and thickness[ilayer] < epsilon:
            ...
            ccc[0, 0] = ((1.+r3)*CaCb-XaXb-r3*YaYb-2.*r1*ex)*r2
            ...
            ccc[2, 0] = 2.*mu2*r2*(r1*r3*YaYb-(CaCb-ex)*(r3+r1)+XaXb)
            ...
            ccc[4, 0] = mu2*mu2*r2*(2.*(CaCb-ex)*r3-XaXb-r3*r3*YaYb)
            ccc[4, 1] = ccc[3, 0]
            ccc[4, 2] = -ccc[2, 0]/2.
            ccc[4, 3] = ccc[1, 0]
            ccc[4, 4] = ccc[0, 0]
        ...
        if ilayer == src_layer:
            ...
            if updn == "all":
                ...
                sss[:nsrc_type+1, :4] = (np.asarray(si)[:nsrc_type+1, :4]@np.asarray(temppp).T)/2.
                sss[:nsrc_type+1, 4] = (np.asarray(si)[:nsrc_type+1, 4]+temp_sh*np.asarray(si)[:nsrc_type+1, 5])/2.
                sss[:nsrc_type+1, 5] = (np.asarray(si)[:nsrc_type+1, 5]+np.asarray(si)[:nsrc_type+1, 4]/temp_sh)/2
            # * end separatS
            ...
            for index in range(3):
                ...
                zzz[index,0]=-sss[index,1]*ggg[0]-sss[index,2]*ggg[1]+sss[index,3]*ggg[2]
                zzz[index,1]= sss[index,0]*ggg[0]-sss[index,2]*ggg[2]-sss[index,3]*ggg[3]
                zzz[index,2]= sss[index,0]*ggg[1]+sss[index,1]*ggg[2]-sss[index,3]*ggg[4]
                zzz[index,3]=-sss[index,0]*ggg[2]+sss[index,1]*ggg[3]+sss[index,2]*ggg[4]
                # c for sh
    ...
    for index in range(3):
        val = zzz_np[index, 1]*ggg_np[0]+zzz_np[index, 2]*ggg_np[1]-zzz_np[index, 3]*ggg_np[2]
        zzz_np[index, 1] = -zzz_np[index, 0]*ggg_np[0]+zzz_np[index, 2]*ggg_np[2]+zzz_np[index, 3]*ggg_np[3]
        zzz_np[index, 0] = val
